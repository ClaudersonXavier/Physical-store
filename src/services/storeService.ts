...
const createStoreByCep = async (data: any, cep: string) =>{
   try{
       ...
       const store = new Store(newData);
        ...
        await store.save();
        logger.info("Loja criada com o CEP.")
        return store;  
...
const getAllStores = async () => {
    try{
        ...
        logger.info("Retornando todas as lojas")
        return stores;
    }catch(error){
...
const getStoreById = async (id: string) => {
    try{
        ...
        logger.info("Retornando a loja encontrada.");
        return store;
    }catch(error){
...
const updateStore = async (id: string, body: any) => {
    try{
        ...
        if(store.endereco!.CEP !== updateStore!.endereco?.CEP){
...
const findNearbyStores = async (cep: string) => {
   try{
       ...
       logger.info("Busca de lojas próximas concluida.");
       return nearbyStores;
   }catch(error){
